...
int MIP_fitter_min_U(){

TFile *f = new TFile("/path_to_input.root"); 
f->ls();

ofstream file;
file.open("fit_parm_U_min.txt");
...
*/
    ...
    std::vector<double> roots = {0,0,0,-25};
...
//    if( n>500 && n<1900){
//    file << n << " " << (minima*1.2) << " " << chi2 << " " << ndf << " " << chi2/ndf << " " << noise_height << " " << noise_peak << endl;}
//    else {
file << n << " " << minima << " " << chi2 << " " << ndf << " " << chi2/ndf << " " << noise_height << " " << noise_peak << endl;//}
}
